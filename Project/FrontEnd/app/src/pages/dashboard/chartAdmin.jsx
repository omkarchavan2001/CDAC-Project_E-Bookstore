...
export default function ChartAdmin({data,daily,monthly}){
    
    return (
        <div style={{ padding: '20px' }}>
                  <div>
            <div className="d-flex justify-content-between">
                {monthly!=null&&<div className='h3 border p-3 rounded border-dark'>Monthy Earnings: ₹{(monthly*.25).toFixed(2)}</div>}
                {daily!=null&&<div  className='h3 border p-3 rounded border-dark'>Today's earnings: ₹{(daily*.25).toFixed(2)}</div>}
            </div>
            <div className='mt-5'>
